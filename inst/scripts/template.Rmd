---
title: "CZe1 SFB3B1 WT/KO peaks QC and downstream analysios"
author: "John Doe"
date: "`r Sys.Date()`"
output:
  html_document:
...
# import nf sample sheet, tidy up and save as 'sample_df.rds' in the
# data directory

# sample sheet should have 'antibody' column
sample_df <- read_csv(file.path(result_dir, 'data', 
                            'nf-sample-sheet.csv')) 

# tidy up: if the sample sheet has antibody column                            
sample_df <- sample_df  %>%
...
The code chunk below uses `peakable:::peakle_flow()` to import peak ranges (`grl`) and information (`df`) of bed/bam files paths and sample metadata (extracted from the _Nextflow_ sample sheet). Note `bam_pattern` was default fo '\\.markedDup.filter.sort.bam\$'. The `bed_pattern` and `bam_pattern` must include characters that can be removed for the files in order to restore the `sample_id` defined in nextflow sample sheet (`sample_df\$sample_id`).
...
macs2 <- # return: df and grl 
  peakable:::peakle_flow(sample_df = narrow_df, 
