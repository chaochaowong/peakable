...
result_dir <- 
  file.path(dir_header) # add your mount to the result directory
...
```{r nf-sample-sheet-to-rds, eval=FALSE}
...
.get_consensus_name <- function(df) {
  df %>%
    group_by(antibody, cell_line) %>%
    summarize(antibody=unique(antibody), cell_line= unique(cell_line)) %>%
    dplyr::mutate(consensus = paste0(antibody, "-", cell_line)) %>%
    pull(consensus)
}
```
...
- The path to `bam` files is set to `result_dir/samtools_sort` (I might in the futhure allow user to define the bam file path)
...
```{r macs-consensus}
# consensus
macs2_consensus <- list()
macs2_consensus$grl <- macs2$df %>%
  group_split(antibody, cell_line) %>%
  map(function(df) {
    x <- macs2$grl[[df$sample_id[1]]]
    y <- macs2$grl[[df$sample_id[2]]]
    if (length(x) > 0 & length(y) > 0)
        peakable::find_consensus_macs2(x, y)
    else GRanges()
  })
names(macs2_consensus) <- .get_consensus_name(macs2$df)
macs2_consensus$df <- 
  data.frame(sample_id = names(macs2_narrow_consensus$grl),
             number_of_peaks = sapply(macs2_narrow_consensus$grl, length))
# hints: better add cell_line and antibody to macs2_consensus$df
